...
// =========================================================
// adminsdashboard.js (FIXED & COMPLETE)
// =========================================================

// Assuming displayMessage is defined to show feedback to the user
...
// ----------------------------------------------------------------
// 1. FIX FOR: ReferenceError: token is not defined
// ----------------------------------------------------------------
/**
 * Retrieves the JWT from localStorage.
 * Adjust 'jwtToken' if your storage key is different.
 */
...
// ----------------------------------------------------------------
// 2. Main Submission Handler (Includes 400 fix logic)
// ----------------------------------------------------------------
async function membershipFormSubmitHandler(event) {
    // Line 478 (Approx)
    event.preventDefault(); 
    
    try {
        // --- CRITICAL TOKEN FIX ---
        const token = getAuthToken(); // ✅ Define 'token' by calling the helper function
        if (!token) {
            displayMessage("Error: Authentication token not found. Please log in again.", "error");
            return;
        }
        
        // --- DATA EXTRACTION (Fix for earlier 400 Bad Request) ---
        const button = event.target;
        
        // 1. Extract Request ID
        const requestIdElement = button.closest('[data-request-id]');
        if (!requestIdElement) throw new Error("Could not find Request ID on the element.");
        
        const requestId = parseInt(requestIdElement.getAttribute('data-request-id'), 10);
        
        // 2. Extract Action
        const action = button.getAttribute('data-action'); 

        if (isNaN(requestId) || !action) {
            throw new Error("Invalid Request ID or Action data missing from the button.");
        }
        ...
        const response = await fetch('/api/admin/requests', {
            ...
            headers: {
                ...
                // Line 480 (Approx) - Using the defined 'token' variable
                'Authorization': `Bearer ${token}` 
            },
        ...
        // Placeholder for refreshing the list (you must implement this function)
        ...
        // Line 503 (Approx)
        displayMessage(`Failed to submit membership: ${error.message}`, "error");
    }
}

// ----------------------------------------------------------------
// 3. EVENT LISTENER SETUP (Example)
// ----------------------------------------------------------------
document.addEventListener('DOMContentLoaded', () => {
    // Attach handler to all buttons with a specific class that handle admin actions
    // You need to ensure your HTML uses this class for your Approve/Reject buttons
    document.querySelectorAll('.admin-action-button').forEach(button => {
        // Ensure you don't attach the listener twice if the button is dynamically added
        button.removeEventListener('click', membershipFormSubmitHandler);
        button.addEventListener('click', membershipFormSubmitHandler);
    });
    
    // Placeholder to load initial data
    // if (typeof loadCourts === 'function') loadCourts();
});

// ----------------------------------------------------------------
// 4. Other Required Functions (Placeholders)
// ----------------------------------------------------------------
// You must define these functions elsewhere in adminsdashboard.js
// function loadCourts() { /* ... implementation ... */ }
// function loadPendingRequests() { /* ... implementation ... */ }