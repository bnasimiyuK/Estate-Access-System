...
async function membershipFormSubmitHandler(e) {
  ...
  if (!payload.RequestID || !payload.Action) {
    displayMessage("Request ID and Action are required.", true);
    if (submitBtn) submitBtn.disabled = false;
    return;
  }

  try {
    const res = await fetch(`${API_HOST}/api/admin/requests`, {
      method: "POST",
      headers: getAuthHeaders(),
    ...
    displayMessage("Membership request submitted successfully.", false);
    form.reset();
  } catch (err) {
