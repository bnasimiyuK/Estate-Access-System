...
const displayFeedback = (message, isSuccess = false) => {
    ...
    feedbackMessage.classList.add(isSuccess ? 'text-green-700' : 'text-red-700', isSuccess ? 'bg-green-100' : 'bg-red-100');
...
const fetchCourts = async () => {
    try {
        // Correctly reset for loading state
        courtNameSelect.innerHTML = '<option value="">Loading courts...</option>'; 
        const res = await fetch(ENDPOINTS.fetchCourts);
        
        if (!res.ok) {
            // Include server response details if available
        ...
        // Set default option after successful fetch
        courtNameSelect.innerHTML = '<option value="">Select Court</option>';

        courts.forEach(court => {
            const option = document.createElement('option');
            // Ensure the value matches the data from the server
        ...
        courtNameSelect.innerHTML = '<option value="" disabled>Failed to load courts</option>';
        displayFeedback(`Failed to load court names: ${err.message}`, false);
    }
};
