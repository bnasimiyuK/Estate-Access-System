...
async function fetchMembershipRecords() {
  try {
    ...
    // Track unique filter values to prevent duplicates
    ...
    records.forEach(record => {
      ...
      tr.innerHTML = `
        ...
        <td class="px-4 py-2">${record.RequestedAt ? new Date(record.RequestedAt).toLocaleString() : ""}</td>
      ...
      if (!residentNames.has(record.ResidentName)) {
        ...
        residentNames.add(record.ResidentName);
      }
    });
