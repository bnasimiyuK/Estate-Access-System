...
export async function handleAdminRequest(req, res) {
    try {
        const { requestId, action } = req.body; // Expecting { requestId: 123, action: 'Approve' }
        ...
        const statusToSet = action === 'Approve' ? 'Approved' : (action === 'Reject' ? 'Rejected' : null);
        ...
        const updateResult = await pool.request()
            ...
            .query(`
                ...
                SET Status = @Status, ApprovedOrRejectedAt = GETDATE()
                WHERE RequestID = @RequestID AND Status = 'Pending';
            `);

        if (updateResult.rowsAffected[0] === 0) {
            // This happens if the request was already handled or RequestID is invalid
            return res.status(404).json({ message: "Membership request not found or already processed." });
        }
        
        // 2. If approved, you would typically insert the user into the main Residents/Users table here.
        // (Assuming you handle the full approval flow elsewhere or this is just a status update).
