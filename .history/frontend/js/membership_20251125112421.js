...
const TOKEN_KEY = "accessToken";  // Key used in localStorage
const AUTH_TOKEN = localStorage.getItem(TOKEN_KEY); // Actual token value
...
async function loadCourts() {
    ...
    try {
        ...
        const res = await fetch(`${API_HOST}/api/courts/all`, {
            headers: { 
...
async function loadMembershipCount() {
    ...
    try {
        ...
        const res = await fetch(`${API_HOST}/api/admin/membership/count`, {
            headers: { 
                "Authorization": `Bearer ${AUTH_TOKEN}` // ✅ Auth header for GET
            }
        });
...
async function membershipFormSubmitHandler(e) {
    ...
    try {
        const res = await fetch(`${API_HOST}/api/admin/membership`, {
            ...
            body: JSON.stringify(submissionBody)      // ✅ Convert JS object to JSON
        });
