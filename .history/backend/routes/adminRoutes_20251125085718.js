...
import { handleAdminRequest } from "../controllers/adminController.js";
...
router.get("/dashboard/summary", authenticateJWT, isAdmin, async (req, res) => {
    // Example of how to prevent caching for a specific route
router.get("/dashboard/summary", authenticateJWT, isAdmin, async (req, res) => {
    // ⚠️ Add these lines to explicitly prevent caching in the response headers
...
router.get("/dashboard/accesschart", authenticateJWT, isAdmin, async (req, res) => {
    // ⚠️ ANTI-CACHING FIX: Ensure the browser fetches new data every time
    res.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
    res.set('Pragma', 'no-cache');
    res.set('Expires', '0');
    try {
...
router.post("/requests", authenticateJWT, isAdmin, handleAdminRequest);
