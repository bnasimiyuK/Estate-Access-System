...
export const getResidents = async (req, res) => {
  const { role, userId } = req.user; // Extracted from JWT middleware
  ...
  try {
    ...
    if (role && role.toLowerCase() === "admin") {
      ...
      result = await pool.request().query(`
        SELECT 
          r.ResidentID, r.UserID, r.ResidentName, r.NationalID, r.PhoneNumber, r.Email,
          r.HouseNumber, r.CourtName, r.Occupation, r.DateJoined, r.Status, u.roleName
        FROM Residents r
        LEFT JOIN Users u ON r.UserID = u.UserID
        ORDER BY r.ResidentID DESC
      `);
    } else if (role && role.toLowerCase() === "resident") {
